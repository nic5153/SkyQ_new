...
from queue_merge.r2_publish import publish_observing_plan
...
def main():
        ...
        print("[1/6] Syncing Cloudflare R2 submissions...", flush=True)
        r2 = sync_r2_submissions()
        summary["r2"] = r2

        if r2["enabled"]:
                print(
                        f"[1/6] R2 sync complete: "
        ...
        print(
                f"[1/6] Known submissions: {submissions['submission_count']}",
                flush=True,
        )

        print("[2/6] Processing observer inbox...", flush=True)
        inbox = process_inbox()
        summary["inbox"] = inbox
        print(
                f"[2/6] Inbox complete: "
                f"{inbox['files_found']} files found, "
                f"{inbox['accepted_files']} accepted, "
                f"{inbox['rejected_files']} rejected",
                flush=True,
        )
        print(f"[2/6] Inbox report: {inbox['report_path']}", flush=True)

        print("[3/6] Building master table...", flush=True)
        master = make_master()

        if master is not None:
                summary["master_count"] = len(master)
                print(f"[3/6] Master table complete: {len(master)} targets", flush=True)
        else:
                print("[3/6] Master table failed or no valid targets found", flush=True)
                finish(summary, status="failed", error="Master table failed or no valid targets found")
                return

        print("[4/6] Building observing plan...", flush=True)
        plan = make_observing_plan()

        if plan is not None:
                summary["plan"] = plan
                print(f"[4/6] Observing plan complete: {len(plan)} targets", flush=True)
                ...
                print("[4/6] Observing plan failed", flush=True)
                finish(summary, status="failed", error="Observing plan failed")
                return

        print("[5/6] Creating observer products...", flush=True)
        products = make_observer_products()
        summary["product_count"] = len(products)
        print(f"[5/6] Observer products complete: {len(products)} targets", flush=True)

        print("[6/6] Publishing observing plan to website storage...", flush=True)
        publish = publish_observing_plan()
        summary["publish"] = publish

        if publish["enabled"] and publish["published"]:
                print(
                        f"[6/6] Published website observing plan: "
                        f"s3://{publish['bucket']}/{publish['latest_key']}",
                        flush=True,
                )
